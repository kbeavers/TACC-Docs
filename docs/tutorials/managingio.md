...
<code>$WORK</code> <sup><a href="#sup1">1</a></sup> | software installations, original datasets that can't be reproduced.  | The Stockyard file system is NOT backed up.<br>Ensure that your important data is backed up to <a href="/user-guides/ranch">Ranch</a> long-term storage.
NOWRAP<code>$SCRATCH</code> <sup><a href="#sup2">2</a></sup>ESPAN | Reproducible datasets, I/O files: temporary files, checkpoint/restart files, job output files | Not backed up.<br>All <code>$SCRATCH</code> file systems are <b>subject to purge</b> if access time <sup><a href="#sup3">3</a></sup> is more than 10 days old.
...
SDL what's this = File.read "../include/tinfo.html"
...
fault stripe count on TACC file systems is 1. Please keep using a single stripe for small files but stripe large files.
